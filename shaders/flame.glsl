...
void main() {
	...
	if( gl_LocalInvocationID.x == 0 ) {
		/*if(first_run) {
			xid = (use_random_xform_selection)? get_xform_id(randf()): get_xform_id(float(gl_WorkGroupID.x) / float(gl_NumWorkGroups.x));
			xids[gl_WorkGroupID.x] = xid;
		}
		else xid = (use_random_xform_selection)? get_xform_id(randf()): xids[gl_WorkGroupID.x];*/
	...
	if(do_draw) {
		//result.xy = vec2(cos_rot * result.x - sin_rot * result.y, sin_rot * result.x + cos_rot * result.y);
